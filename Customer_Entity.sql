...
drop stream stage_sch.customer_stm;
drop table clean_sch.customer;
drop stream clean_sch.customer_stm;
...
//select * from consumption_sch.customer_dim order by customer_id;
// delta processing check

list @stage_sch.csv_stg/delta/customer/;

//csv_stg/delta/customer/day-01-insert-customer.csv
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @csv_stg/delta/customer/day-02-insert-update.csv t
)
file_format = (format_name = 'stage_sch.csv_file_format')
on_error = abort_statement;
