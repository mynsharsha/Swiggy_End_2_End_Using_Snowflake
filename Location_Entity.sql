...
--query external location stage by adding audit columns
select 
    t1.$1::text as location_id,
    t1.$2::text as city,
    t1.$3::text as state,
...
(
    ...
    _stg_file_md5 text,
    _copy_date_ts  timestamp default current_timestamp
);

//Stream - this is append only stream - object that get delta based data
// delta - captures changed/modified data where as initial captures the original data.

create or replace stream stage_sch.location_stm
on table stage_sch.location
append_only = true;

copy into stage_sch.location(locationid,city,state,zipcode,activeflag,createdate,modifieddate,_stg_file_name,_stg_file_load_ts,_stg_file_md5,_copy_date_ts)
from(
select 
    t1.$1::text as location_id,
    t1.$2::text as city,
...
_copy_date_ts timestamp_ntz default current_timestamp
);
//drop table clean_sch.restaurant_location;

//CREATING STREAM ON THIS TABLE
...
WHEN MATCHED AND (
    ...
    UPDATE SET 
        ...
        target._copy_date_ts = source._copy_data_ts
...
alter schema
consumption rename to consumption_sch; 
