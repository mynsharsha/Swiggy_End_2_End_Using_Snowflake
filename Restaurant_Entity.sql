...
drop  SANDBOX.CLEAN_SCH.RESTAURANT_STM;
...
--restaurant where data types are defined.
...
create stream clean_sch.restaurant_stm
...
select * from SANDBOX.CLEAN_SCH.RESTAURANT_STM;
...
// loading the delta data

list @"SANDBOX"."STAGE_SCH"."CSV_STG"/delta/restaurant/day-02-upsert-restaurant-delhi+NCR.csv;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp() as _copy_data_ts
     from @"SANDBOX"."STAGE_SCH"."CSV_STG"/delta/restaurant/day-02-upsert-restaurant-delhi+NCR.csv t
)
file_format = (format_name = 'stage_sch.csv_file_format')
on_error = abort_statement;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp() as _copy_data_ts
     from @csv_stg/delta/restaurant/day-01-insert-restaurant-delhi+NCR.csv t
